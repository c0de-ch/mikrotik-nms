...
- **`internal/poller/manager.go`** — orchestrates background polling goroutines:
  - Health: 30s (env), polls `/system/resource` for all devices. A device is only marked `offline` after it has missed polls for longer than `offline_threshold_seconds` (app_setting, default 120s); within that grace window it keeps its last-known `online` status to avoid flapping on transient failures. `last_seen` means *last successful contact* (not bumped on failed polls).
