...
- App settings (`app_settings` table) are runtime-tunable via the **Settings** page; backend pollers that read from this table (wifi_interval, client_discovery_interval, kea_url, DNS servers, port_monitor_*, offline_threshold_seconds, info_interval) pick up changes on the next cycle without a restart. Pollers driven by env vars (health, topology, firmware, network_health, retention) require a backend restart.
